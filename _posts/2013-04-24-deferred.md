...
The one exception is the "jQuery.Deferred - chainability" which assumes that, when member functions are invoked with any context (even when it's not the deferred object), will both execute successfully and will return the invocation's context:
